...
Write-Host "   Chatwoot Flutter Web 自动部署" -ForegroundColor Cyan
Write-Host "================================================" -ForegroundColor Cyan
Write-Host ""

# 步骤 1: 选择部署环境
Write-Host "[1/6] 选择部署环境..." -ForegroundColor Yellow
Write-Host "  1. 测试环境 (testing)"
Write-Host "  2. 生产环境 (production)"
Write-Host ""
$envChoice = Read-Host "请选择环境 (1-2，默认1)"
if ([string]::IsNullOrWhiteSpace($envChoice)) { $envChoice = "1" }

$envParam = ""
$envName = ""
if ($envChoice -eq "2") {
    $envParam = "--dart-define=ENV=production"
    $envName = "生产环境"
    Write-Host "  已选择: 生产环境" -ForegroundColor Green
    Write-Host "  ⚠️  请确保已配置生产环境的 API 地址和 Token！" -ForegroundColor Yellow
} else {
    $envParam = "--dart-define=ENV=testing"
    $envName = "测试环境"
    Write-Host "  已选择: 测试环境" -ForegroundColor Green
}
Write-Host ""

# 步骤 2: 构建 Flutter Web
Write-Host "[2/6] 构建 Flutter Web 应用 ($envName)..." -ForegroundColor Yellow
Write-Host ""
...
flutter build web --release --web-renderer html $envParam
...
# 步骤 3: 打包构建产物
Write-Host "[3/6] 打包构建产物..." -ForegroundColor Yellow
...
# 步骤 5: 在服务器上部署
Write-Host "[5/6] 配置服务器..." -ForegroundColor Yellow
